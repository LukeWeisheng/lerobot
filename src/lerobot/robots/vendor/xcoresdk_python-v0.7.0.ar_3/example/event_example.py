...
def print_log_reporter(info):
    ...
    ecode = info[xCoreSDK_python.EventInfoKey.LogReporter.Ecode]
    edetail = info[xCoreSDK_python.EventInfoKey.LogReporter.Edetail]
    print(f"logReporter ecode:{ecode}")
    print(f"logReporter edetail:{edetail}")
    ...
    loginfos = robot.queryControllerLog(
        ...
        ec,
    )  # 最多查询最近十条日志，实际不需要这么多,也可以根据情况只查询特定级别的日志
...
def set_log_reporter_event(robot: xCoreSDK_python.BaseRobot, ec):
    ...
    robot.setEventWatcher(xCoreSDK_python.Event.logReporter, print_log_reporter, ec)
    print("按下 'q' 键退出循环")
    while True:
        user_input = input("输入：")
        if user_input.strip() == "q":
            robot.setNoneEventWatcher(
                xCoreSDK_python.Event.logReporter, ec
            )  # 程序退出前需要取消事件监听
            break
