...
def example_CAN(robot, ec):
    '''CAN通信'''
    print_separator("example_CAN", length=80)
    
    send_frames = []
    send_frame = xCoreSDK_python.CANFrame()
    send_frame.frame_id = 0x27
    send_frame.frame_format = 0
    send_frame.frame_type = 2
    send_frame.frame_valid_length = 7
    send_frame.data = [1, 50, 50, 50, 50, 50, 50]
    send_frames.append(send_frame)
    robot.CANSendData("uint8", send_frames, ec)
    print_log("CANSendData", ec)
    
    time.sleep(0.5)

    send_frames.clear()
    send_frame.data = [1, 255, 255, 255, 255, 255, 255]
    send_frames.append(send_frame)
    robot.CANSendData("uint8", send_frames, ec)
    
    recv_frame = xCoreSDK_python.CANFrame()
    robot.CANReceiveData(50, "uint8", recv_frame, ec)
    print(f"CANReceiveData: {ec} {recv_frame}")
